...
while (mpciter < loop_run)
    ...
    mpciter
    mpciter = mpciter + 1;
    a_p(mpciter) = (x0(3)*(tan(x0(7)+VFOV/2)) - x0(3)*tan(x0(7)-VFOV/2))/2; %this is for plotting FOV
    b_p(mpciter) = (x0(3)*(tan(x0(6)+HFOV/2)) - x0(3)*tan(x0(6)-HFOV/2))/2;
    X_E_1(mpciter) = x0(1) + a_p(mpciter) + x0(3)*(tan(x0(7)-VFOV/2));
    Y_E_1(mpciter) = x0(2) + b_p(mpciter) + x0(3)*(tan(x0(6)-HFOV/2));
end
...
x_e = zeros(loop_run,101);
y_e = zeros(loop_run,101);

%% Plotting stuff----------------------------------------------------------------------------------------------------------------------
for i=1:loop_run
    [x_e(i,:), y_e(i,:)] = ellipse(a_p(i), b_p(i), X_E_1(i), Y_E_1(i)); %this is for plotting FOV
end
...
plot3(ss(1,1:loop_run),ss(2,1:loop_run), ss1(1,1:loop_run),'b--',...
    ...
    x_e(loop_run,1:101), y_e(loop_run,1:101), ss1(1,1:101), 'y-', 'LineWidth', 2);
hold on;
...
figh = figure('position', [10 10 1300 650])
for j=1:loop_run
    plot3(ss(1,1:j),ss(2,1:j), ss1(1,1:j),'b--',...
    xx(1,1:j),xx(2,1:j),ss1(1,1:j), 'g-',...
    x_e(j,1:101), y_e(j,1:101), ss1(1,1:101), 'y-', 'LineWidth', 2);
    hold on;
    plot3(xx(1,1:j),xx(2,1:j), xx(3,1:j),'r-', 'LineWidth', 3)
    hold on;
    surf(X+x_o_1,Y+y_o_1,Z);
    hold on;
    surf(X+x_o_2,Y+y_o_2,Z);
...
myWriter = VideoWriter('Track_1_updated', 'MPEG-4');
