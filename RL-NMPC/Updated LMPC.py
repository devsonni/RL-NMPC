...
qtable1 = np.zeros((step_size, 101, 101))
qtable2 = np.zeros((step_size, 101, 101))
qtable3 = np.zeros((step_size, 101, 101))
qtable4 = np.zeros((step_size, 101, 101))
qtable5 = np.zeros((step_size, 101, 101))

# Q - learning parameters
total_episodes = 150 # 50 Total episodes
...
UAV_RL_1 = ca.DM.zeros((27, max_step_size+1))
...
for episode in range(total_episodes):
    ...
    while not done:
        ...
        if exp_exp_tradeoff > epsilon:
            ...
            action2 = max_index(qtable1[step, :, :])
            action3 = max_index(qtable1[step, :, :])
        ...
        # Else doing a random choice --> exploration
            ...
            action = (0, 0, 0, 0, 0, 0)
            while(action[0] == 0 or action[1] == 0 or action[2] == 0 or action[3] == 0 or action[4] == 0 or action[5] == 0):
        ...
        reward_t = reward + reward1 + reward2
        ...
        qtable1[step, action[2], action[3]] = (1 - learning_rate) * (qtable1[step, action[2], action[3]]) + \
                                             learning_rate * (reward1 + gamma * np.max(qtable1[step + 1, :, :]))

        qtable2[step, action[4], action[5]] = (1 - learning_rate) * (qtable2[step, action[4], action[5]]) + \
                                              learning_rate * (reward2 + gamma * np.max(qtable2[step + 1, :, :]))

        qtable3[step, action[0], action[1]] = (1 - learning_rate) * (qtable3[step, action[0], action[1]]) + \
                                              learning_rate * (reward_t + gamma * np.max(qtable3[step + 1, :, :]))

        qtable4[step, action[2], action[3]] = (1 - learning_rate) * (qtable4[step, action[2], action[3]]) + \
                                              learning_rate * (reward_t + gamma * np.max(qtable4[step + 1, :, :]))

        qtable5[step, action[4], action[5]] = (1 - learning_rate) * (qtable5[step, action[4], action[5]]) + \
                                              learning_rate * (reward_t + gamma * np.max(qtable5[step + 1, :, :]))
...
#############################################################
#############################################################
# error of seconde optimal policy
Error = 0
env.reset()
UAV_RL_1[:, 0] = x0[0:27]
targetarr[:, 0] = xs
#Printing Optimal Policy
for i in range(max_step_size):
    action1 = max_index(qtable3[i, :, :])
    action2 = max_index(qtable4[i, :, :])
    action3 = max_index(qtable5[i, :, :])
    action = (action1[0], action1[1], action2[0], action2[1], action3[0], action3[1])
    new_state, obs2, reward, reward1, reward2, done, info, error, error1, error2, error3, a_p, b_p, x_e, y_e, a_p_1, \
    b_p_1, x_e_1, y_e_1, a_p_2, b_p_2, x_e_2, y_e_2 = env.step(action)
    UAV1_RL_Err_1[i] = error
    UAV2_RL_Err_1[i] = error1
    UAV3_RL_Err_1[i] = error2
    UAV_RL_1[:, i+1] = new_state
    # targetarr[:, i+1] = obs2
    # maxreward[i] = reward
    minerrorarr_1[i] = error3
    # Error += (error + error3)
    print(i)
...
np.savez("Jo_test_150ep_final_high_speed.npz", errorarr=errorarr, w_1=w_1, w_2=w_2, w_3=w_3, w_4 = w_4, w_5=w_5, w_6=w_6, erroravg=erroravg,
...
x_axis_bar = ['Error without RL', 'Error of FirstEpisode', 'Error of LastEpisode', 'Error of OptimalPolicy', 'Error of OptimalPolicy 1']
...
mlab.plot3d(UAV_RL[0+8, 0:max_steps], UAV_RL[1+8, 0:max_steps], UAV_RL[2+8, 0:max_steps], tube_radius=5, color=(255/255, 166/255, 0))
mlab.plot3d(UAV_W_RL[0+8, 0:max_steps], UAV_W_RL[1+8, 0:max_steps], UAV_W_RL[2+8, 0:max_steps], tube_radius=5, color=(0,1,0))
mlab.plot3d(UAV_RL[0+16, 0:max_steps], UAV_RL[1+16, 0:max_steps], UAV_RL[2+16, 0:max_steps], tube_radius=5, color=(255/255, 166/255, 0))
mlab.plot3d(UAV_W_RL[0+16, 0:max_steps], UAV_W_RL[1+16, 0:max_steps], UAV_W_RL[2+16, 0:max_steps], tube_radius=5, color=(0,1,0))

mlab.plot3d(UAV_RL[0, 0:max_steps], UAV_RL[1, 0:max_steps], ss1[0:max_steps], tube_radius=4, color=(255/255, 166/255, 0))
mlab.plot3d(UAV_W_RL[0, 0:max_steps], UAV_W_RL[1, 0:max_steps], ss1[0:max_steps], tube_radius=4, color=(0,1,0))
mlab.plot3d(UAV_RL[0+8, 0:max_steps], UAV_RL[1+8, 0:max_steps], ss1[0:max_steps], tube_radius=4, color=(255/255, 166/255, 0))
mlab.plot3d(UAV_W_RL[0+8, 0:max_steps], UAV_W_RL[1+8, 0:max_steps], ss1[0:max_steps], tube_radius=4, color=(0,1,0))
mlab.plot3d(UAV_RL[0+16, 0:max_steps], UAV_RL[1+16, 0:max_steps], ss1[0:max_steps], tube_radius=4, color=(255/255, 166/255, 0))
mlab.plot3d(UAV_W_RL[0+16, 0:max_steps], UAV_W_RL[1+16, 0:max_steps], ss1[0:max_steps], tube_radius=4, color=(0,1,0))
...
mlab.plot3d(targetarr[0+3+3,0:max_steps], targetarr[1+3+3,0:max_steps], ss1[0:max_steps], tube_radius=5, color=(0,.7,0))
...
mlab.plot3d(x_e_rl_2[0:100, 0], y_e_rl_2[0:100, 0] , ss1[0:100], tube_radius=3, color=(0,0,0))
mlab.plot3d(x_e_w_rl_2[0:100, 0], y_e_w_rl_2[0:100, 0] , ss1[0:100], tube_radius=3, color=(1,1,1))
